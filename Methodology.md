...
| Newfoundland & Labrador | 0 | 0 | 39 | 361 |
...
One province (Newfoundland & Labrador) shows zero animals in the "PA only" and "G+PA" groups — every animal in its file is coded `EBV`, which given the pattern in every other province is not realistic and points to that source file not distinguishing `LPI Code` correctly (see Section 4.1). The other 9 provinces show a believable mix across all four groups.
