...
**Practical conclusion for the project:** don't claim a conformation-to-udder-health link - the data doesn't support it. The `DS`-`BMR` relationship is worth reporting as an expected, internally consistent result, not a health/longevity finding.
...
**Important context confirmed against Lactanet's own published LPI structure (source: "Which Index is Right for My Herd?", Lactanet, March 2025):** LPI is officially calculated as a weighted sum of six subindexes: Production Index (PI, 40%), Longevity and Type Index (LTI, 32%), Health and Welfare Index (HWI, 8%), Reproduction Index (RI, 10%), Milkability Index (MI, 5%), and Environmental Impact Index (EI, 5%). PI is itself built from Milk, Fat, and Protein yield. LTI is built from Herd Life, Conformation, Mammary System, Feet and Legs, Dairy Strength, and Rump.

Nearly every trait used in this regression (`Milk`, `Fat`, `Prot`, `Conf`, `MS`, `F&L`, `DS`, `RP`, `HWI`, `RI`, `MI`, `EI`) is therefore a direct structural component of LPI, not an independent variable being tested against it. The high R² found for this model (0.984) should be read mainly as confirming the known, published structure of the LPI formula, not as an independent empirical discovery. This is a broader version of the multicollinearity issue described below for `Conf` specifically: here the issue isn't just correlation between predictors, it's that most of the predictors are literally the ingredients LPI is computed from.
...
**On the `DS`-`BMR` relationship (Section 4.2, r=-0.59):** an earlier draft of this analysis incorrectly identified `BMR` as a mastitis-resistance trait and cited resource-allocation trade-off theory (Rauw et al., 1998) to explain it as an immune-function trade-off. That citation has been removed: `BMR` is Body Maintenance Requirements, a feed-cost trait unrelated to disease resistance, and the correct explanation is much simpler and doesn't require an external biological framework. Per Lactanet's own description of this trait, maintenance energy is proportional to an animal's metabolic body weight, so animals with more body capacity (`DS`) mechanically require more maintenance feed (equivalent to a lower `BMR` value, since higher `BMR` means lower requirements). The negative correlation is therefore an expected consequence of how the two traits are defined, not an independent biological discovery.
...
These are cited for general methodological and biological context. The Sweett (2025) reference specifically was used to verify and correct a project finding, not just for background: see Sections 5 and 5.1.
