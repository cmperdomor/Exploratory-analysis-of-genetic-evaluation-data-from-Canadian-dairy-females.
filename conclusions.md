...
Separately, checking which traits carry the most weight in a regression predicting LPI found a very high R² (0.984), but this needs an important caveat confirmed against Lactanet's own published LPI structure: LPI is officially built as a weighted sum of six subindexes (Production Index 40%, Longevity and Type Index 32%, Health and Welfare Index 8%, Reproduction Index 10%, Milkability Index 5%, Environmental Impact Index 5%), and nearly every trait used in this regression (Milk, Fat, Prot, Conf, MS, F&L, DS, RP, HWI, RI, MI, EI) is a direct structural ingredient of one of those subindexes. The high R² mostly confirms this known, published formula structure rather than revealing something new. An earlier version of this analysis also reported Milk Yield's contribution as "near zero" (a partial regression coefficient of 0.02), but Lactanet's own published correlation between Milk Yield and LPI is a real, moderate 0.43. The near-zero coefficient reflects multicollinearity between Milk, Fat, and Protein (all three come from the same subindex), not evidence that milk volume is unimportant to LPI. Fat and Protein are the most "solved" traits in the population (only 0.2% of animals fall below breed average on either, versus 22.6% for Dairy Strength). Average LPI rose across birth cohorts from the ~3,400s (2019-2021) to ~3,800 (2025-2026). Feed efficiency showed no meaningful link to body size or conformation (r around 0.02 to 0.14).
